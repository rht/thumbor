        ...
        for source_image_width, source_image_height in SOURCE_IMAGE_SIZES:
            for watermark_source_image_width, watermark_source_image_height in WATERMARK_IMAGE_SIZES:
                for w_ratio, h_ratio in RATIOS:
                    max_width = source_image_width * (old_div(float(w_ratio), 100)) if w_ratio else float('inf')
                    max_height = source_image_height * (old_div(float(h_ratio), 100)) if h_ratio else float('inf')
                    w_ratio = old_div(float(w_ratio), 100.0) if w_ratio else False
                    h_ratio = old_div(float(h_ratio), 100.0) if h_ratio else False

                    ratio = old_div(float(watermark_source_image_width), watermark_source_image_height)
                    ...
                    watermark_image = old_div(float(watermark_image_width), watermark_image_height)
